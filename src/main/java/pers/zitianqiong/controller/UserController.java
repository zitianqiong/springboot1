   ...
   @Cacheable(value = "userList", key = "'users'")
    public List<SysUser> getUsers() {
        ...
        if (redisUtil.exists(series)) {
            ...
            userList = jsonArray.toJavaList(SysUser.class);
        } else {
    ...
    public UserDetails getUser(Principal principal) {
        ...
        if (redisUtil.exists("user:" + username)) {
            ...
            user = jsonObject.toJavaObject(SysUser.class);
        } else {
            user = (SysUser) userDetailsService.loadUserByUsername(username);
    ...
    public SysUser postUser(int id) {
        return  (SysUser) userDetailsService.loadUserByUsername(userService.getById(id).getUsername());
    }
